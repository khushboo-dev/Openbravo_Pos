...
public class StockManagement extends JPanel implements JPanelView {
    ...
    private ServiceHQL<EWarehouse> m_sentlocations;
    ...
    public StockManagement(AppView app) {
        ...
        m_sentlocations = m_dlSales.getLocationsService();
        ...
        m_cat = new JCatalog(m_dlSales, app.getPriceListVersion(), new PropertiesConfig(m_dlSystem.getResourceAsXML("Ticket.Buttons")));
    ...
    public void stateToInsert() {
        ...
        m_LocationsModel.setSelectedKey(m_App.getTerminal().getWarehouse());
        m_LocationsModelDes.setSelectedKey(m_App.getTerminal().getWarehouse());
        m_invlines.clear();
    ...
    private void addLine(EProduct oProduct, double dpor, double dprice) {
    ...
    private void incProduct(EProduct product, double units) {
        // precondicion: prod != null

        MovementReason reason = (MovementReason) m_ReasonModel.getSelectedItem();
        addLine(product, units, product.getPriceSell());
    }
    ...
    // precondicion: sCode != null
        ...
        try {
            EProduct oProduct = m_dlSales.getProductInfoByCode(m_App.getPriceListVersion(), sCode);
            if (oProduct == null) {                  
        ...
        try {
            ...
            if (reason == MovementReason.OUT_CROSSING) {
                ...
                saveData(new InventoryRecord(
                        ...
                        (EWarehouse) m_LocationsModel.getSelectedItem(),
                        m_invlines.getLines()
                    ));
                saveData(new InventoryRecord(
                        d, MovementReason.IN_MOVEMENT,
                        (EWarehouse) m_LocationsModelDes.getSelectedItem(),
                        m_invlines.getLines()
                    ));                
            } else {  
                // Es un movimiento
    ...
    private class CatalogListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            ...
            if (sQty != null) {
                ...
                incProduct( (EProduct) e.getSource(), dQty);
                m_jcodebar.setText(null);
            } else {
                incProduct( (EProduct) e.getSource(),1.0);
            }
        }  
    }  
