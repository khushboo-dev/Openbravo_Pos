...
public class AttributeUsePanel extends JPanelTable2 {
    ...
    protected void init() {
        ...
        row = new Row(
                new Field("OBPOS_ATTRIBUTEUSE_ID", Datas.STRING, Formats.STRING),
                new Field("OBPOS_ATRIBUTESET_ID", Datas.STRING, Formats.STRING),
        ...
        Table table = new Table(
                "OBPOS_ATTRIBUTEUSE",
                new PrimaryKey("OBPOS_ATTRIBUTEUSE_ID"),
                new Column("OBPOS_ATTRIBUTESET_ID"),
                new Column("OBPOS_ATTRIBUTE_ID"),
                new Column("LINENO"));

        lpr = row.getListProvider(app.getSession(),
                "SELECT ATTUSE.OBPOS_ATTRIBUTEUSE_ID, ATTUSE.OBPOS_ATTRIBUTESET_ID, ATTUSE.OBPOS_ATTRIBUTE_ID, ATTUSE.LINENO, ATT.NAME " +
                "FROM OBPOS_ATTRIBUTEUSE ATTUSE, OBPOS_ATTRIBUTE ATT " +
                "WHERE ATTUSE.OBPOS_ATTRIBUTE_ID = ATT.OBPOS_ATTRIBUTE_ID AND ATTUSE.OBPOS_ATTRIBUTESET_ID = ? ORDER BY LINENO", filter);
