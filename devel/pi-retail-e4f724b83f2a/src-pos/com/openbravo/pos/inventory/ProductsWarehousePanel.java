...
import com.openbravo.data.user.SaveProviderCreator;
...
public class ProductsWarehousePanel extends JPanelTable2 {
    ...
    protected void init() {   
        ...
        lpr = new ListProviderCreator(new PreparedSentence(app.getSession(),
                "SELECT L.ID, P.OBPOS_PRODUCTS_ID, P.REFERENCE, P.NAME," +
                "L.STOCKSECURITY, L.STOCKMAXIMUM, COALESCE(S.SUMUNITS, 0) " +
                "FROM OBPOS_PRODUCTS P " +
                "LEFT OUTER JOIN (SELECT OBPOS_STOCKLEVEL_ID, OBPOS_PRODUCTS_ID, OBPOS_LOCATIONS_ID, STOCKSECURITY, STOCKMAXIMUM FROM OBPOS_STOCKLEVEL WHERE OBPOS_LOCATIONS_ID = ?) L ON P.OBPOS_PRODUCTS_ID = L.OBPOS_PRODUCTS_ID " +
                "LEFT OUTER JOIN (SELECT OBPOS_PRODUCS_ID, SUM(UNITS) AS SUMUNITS FROM OBPOS_STOCKCURRENT WHERE OBPOS_LOCATIOS_ID = ? GROUP BY OBPOS_PRODUCTS_ID) S ON P.OBPOS_PRODUCTS_ID = S.OBPOS_PRODUCTS_ID " +
                "ORDER BY P.NAME",
        ...
        SentenceExec updatesent =  new SentenceExecTransaction(app.getSession()) {
            public int execInTransaction(Object params) throws BasicException {
                ...
                if (values[0] == null)  {
                    // INSERT
                    values[0] = StringUtils.newUUID();
                    return new PreparedSentence(app.getSession()
                        , "INSERT INTO OBPOS_STOCKLEVEL (OBPOS_STOCKLEVEL_ID, OBPOS_LOCATIONS_ID, OBPOS_PRODUCTS_ID, STOCKSECURITY, STOCKMAXIMUM) VALUES (?, ?, ?, ?, ?)"
                        , new SerializerWriteBasicExt(row.getDatas(), new int[] {0, 4, 1, 5, 6})).exec(params);
                } else {
                    // UPDATE
                    return new PreparedSentence(app.getSession()
                        , "UPDATE OBPOS_STOCKLEVEL SET STOCKSECURITY = ?, STOCKMAXIMUM = ? WHERE OBPOS_STOCKLEVEL_ID = ?"
                        , new SerializerWriteBasicExt(row.getDatas(), new int[] {5, 6, 0})).exec(params);
                }
            }
        };     
        
        spr = new SaveProviderCreator(updatesent, null, null);
