...
public class JPaymentDebt extends javax.swing.JPanel implements JPaymentInterface {
    ...
    public JPaymentDebt(JPaymentNotifier notifier, ETerminalPayment terminalPayment) {
    ...
    public void activate(EBusinessPartner customer, double dTotal) {
        ...
        if (customer == null) {
            ...
            m_jName.setText(customer.getName());
            m_jNotes.setText(customer.getDescription());
            txtMaxdebt.setText(Formats.CURRENCY.formatValue(RoundUtils.getValue(customer.getCreditLimit())));
            // txtCurdate.setText(Formats.DATE.formatValue(customerext.getCurdate()));
            txtCurdebt.setText(Formats.CURRENCY.formatValue(RoundUtils.getValue(customer.getCreditUsed())));
                
            if (RoundUtils.compare(RoundUtils.getValue(customer.getCreditUsed()), RoundUtils.getValue(customer.getCreditUsed())) >= 0)  {
                m_jKeys.setEnabled(false);
                m_jTendered.setEnabled(false);                
            } else {    
    ...
    public XOrderPayment executePayment() {
        return new XOrderPaymentVoucher(terminalPayment, paid); // "debt" terminal payment or better "" an underpayment
    }
    ...
    private void printState() {
        ...
        if (customer == null) {
            ...
            if (RoundUtils.compare(RoundUtils.getValue(customer.getCreditUsed()) + paid, RoundUtils.getValue(customer.getCreditLimit())) >= 0)  {
                ...
                notifier.setStatus(paid > 0.0 && iCompare <= 0, iCompare == 0);
            }
        }        
    }
