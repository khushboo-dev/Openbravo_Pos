...
//        p.m_iSeq = app.getActiveCash().getSequence();
//        p.m_dDateStart = app.getActiveCash().getStart();
//        p.m_dDateEnd = null;
//        
//        
//        // Pagos
//        Object[] valtickets = (Object []) new StaticSentence(app.getSession()
//            , "SELECT COUNT(*), SUM(OBPOS_PAYMENTS.TOTAL) " +
//              "FROM OBPOS_PAYMENTS, OBPOS_RECEIPTS " +
//              "WHERE OBPOS_PAYMENTS.OBPOS_RECEIPTS_ID = OBPOS_RECEIPTS.OBPOS_RECEIPTS_ID AND OBPOS_RECEIPTS.OBPOS_CLOSEDCASH_ID = ?"
//            , SerializerWriteString.INSTANCE
//            , new SerializerReadBasic(new Datas[] {Datas.INT, Datas.DOUBLE}))
//            .find(app.getActiveCash().getId());
//            
//        if (valtickets == null) {
//            p.m_iPayments = new Integer(0);
//            p.m_dPaymentsTotal = new Double(0.0);
...
//            , "SELECT OBPOS_PAYMENTS.PAYMENT, SUM(OBPOS_PAYMENTS.TOTAL) " +
//              "FROM OBPOS_PAYMENTS, OBPOS_RECEIPTS " +
//              "WHERE OBPOS_PAYMENTS.OBPOS_RECEIPTS = OBPOS_RECEIPTS.OBPOS_RECEIPTS_ID AND OBPOS_RECEIPTS.OBPOS_CLOSEDCASH_ID = ? " +
//              "GROUP BY OBPOS_PAYMENTS.PAYMENT"
//            , SerializerWriteString.INSTANCE
//            , new SerializerReadClass(PaymentsModel.PaymentsLine.class)) //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
//            .list(app.getActiveCash().getId());
//        
//        if (l == null) {
...
//            "SELECT COUNT(DISTINCT OBPOS_RECEIPTS.OBPOS_RECEIPTS_ID), SUM(OBPOS_TICKETLINES.UNITS * OBPOS_TICKETLINES.PRICE) " +
//            "FROM OBPOS_RECEIPTS, OBPOS_TICKETLINES WHERE OBPOS_RECEIPTS.OBPOS_RECEIPTS_ID = OBPOS_TICKETLINES.OBPOS_TICKETS_ID AND OBPOS_RECEIPTS.OBPOS_CLOSEDCASH_ID = ?",
//            SerializerWriteString.INSTANCE,
//            new SerializerReadBasic(new Datas[] {Datas.INT, Datas.DOUBLE}))
//            .find(app.getActiveCash().getId());
//        if (recsales == null) {
...
//            "SELECT SUM(OBPOS_TAXLINES.AMOUNT) " +
//            "FROM OBPOS_RECEIPTS, OBPOS_TAXLINES WHERE OBPOS_RECEIPTS.OBPOS_RECEIPTS_ID = OBPOS_TAXLINES.OBPOS_RECEIPTS_ID AND OBPOS_RECEIPTS.OBPOS_CLOSEDCASH_ID = ?"
//            , SerializerWriteString.INSTANCE
...
//                "SELECT C_TAXCATEGORY.NAME, SUM(OBPOS_TAXLINES.AMOUNT) " +
//                "FROM OBPOS_RECEIPTS, OBPOS_TAXLINES, C_TAX, C_TAXCATEGORY WHERE OBPOS_RECEIPTS.OBPOS_RECEIPTS_ID = OBPOS_TAXLINES.OBPOS_RECEIPTS_ID AND OBPOS_TAXLINES.C_TAX_ID = C_TAX.C_TAX_ID AND C_TAX.C_TAXCATEGORY_ID = C_TAXCATEGORY.C_TAXCATEGORY_ID " +
//                "AND OBPOS_RECEIPTS.OBPOS_CLOSEDCASH_ID = ?" +
//                "GROUP BY C_TAXCATEGORY.NAME"
//                , SerializerWriteString.INSTANCE
