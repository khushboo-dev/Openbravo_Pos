...
public class JCatalog extends JPanel implements ListSelectionListener, CatalogSelector {
    ...
    private Map<String, EProduct> m_productsset = new HashMap<String, EProduct>();
    
    // Set of Categoriespanels
     private Set<String> m_categoriesset = new HashSet<String>();
        
    private ThumbNailBuilder tnbbutton;
    private ThumbNailBuilder tnbbuttoncat;
    private ThumbNailBuilder tnbcat;
    
    private ECategory showingcategory = null;
          
    public JCatalog(DataLogicSales dlSales, String pricelistversion, PropertiesConfig panelconfig) {
        ...
        tnbbuttoncat = new ThumbNailBuilder(Integer.parseInt(panelconfig.getProperty("img-width", "64")),
                Integer.parseInt(panelconfig.getProperty("img-height", "54")),
                "com/openbravo/images/folder_yellow.png");
    }
    ...
    public void loadCatalog() throws BasicException {
        ...
        // Load all categories.
        java.util.List<ECategory> categories = m_dlSales.getRootCategories();
    ...
    protected void fireSelectedProduct(EProduct prod) {
    ...
    private void selectCategoryPanel(String catid) {
        ...
        try {
            ...
            if (!m_categoriesset.contains(catid)) {
                ...
                // Add subcategories
                java.util.List<ECategory> categories = m_dlSales.getSubcategories(catid);
                for (ECategory cat : categories) {

                    jcurrTab.addButton(new ImageIcon(tnbbuttoncat.getThumbNailText(cat.getImage(), cat.getName())), new SelectedCategory(cat));
                }
                
                // Add products
                java.util.List<EProduct> products = m_dlSales.getProductCatalog(pricelistversion, catid);
                for (EProduct prod : products) {
    ...
    private String getProductLabel(EProduct product) {

        if (pricevisible) {
            if (taxesincluded) {
                ETax tax = App.getInstance().getTaxesLogic().getTaxInfo(product.getTaxCategory(), new Date());
    ...
    private void showRootCategoriesPanel() {
        ...
        ECategory cat = (ECategory) m_jListCategories.getSelectedValue();
    ...
    private void showSubcategoryPanel(ECategory category) {
        selectIndicatorPanel(new ImageIcon(tnbbuttoncat.getThumbNail(category.getImage())), category.getName());
    ...
    private void showProductPanel(String id) {
        ...
        EProduct product = m_productsset.get(id);
        ...
        if (product == null) {
            if (m_productsset.containsKey(id)) {
                ...
                try {
                    ...
                    java.util.List<EProduct> products = m_dlSales.getProductComments(id);
                    ...
                    if (products.isEmpty()) {
                        ...
                        // Load product panel
                        product = m_dlSales.getProductInfoById(pricelistversion, id);
    ...
    private class SelectedAction implements ActionListener {
        private EProduct prod;
        public SelectedAction(EProduct prod) {
            this.prod = prod;
        }
    ...
    private class SelectedCategory implements ActionListener {
        ...
        public SelectedCategory(ECategory category) {
