...
import com.openbravo.data.json.HQLParamsCreator;
...
public class JTicketsBagRestaurantRes extends javax.swing.JPanel implements EditorRecord<EReservation> {
    ...
    public JTicketsBagRestaurantRes(AppView oApp, JTicketsBagRestaurantMap restaurantmap) {
        ...
        txtCustomer.addPropertyChangeListener("Text", new PropertyChangeListener(){
            public void propertyChange(PropertyChangeEvent evt) {
                reservation.setBusinessPartner(null);
            }
        });
        ...
        ListProvider<EReservation> lpr = new HQLListProvider<EReservation>(dlCustomers.getReservationsList(), new MyDateCreator());;
        SaveProvider<EReservation> spr = new HQLSaveProvider<EReservation>(App.getInstance().getUser().getHttpSession(), EReservation.getClassEBean());
    ...
    private class MyDateCreator implements HQLParamsCreator {
        public HQLParam[] create() {
            return new HQLParam[]{
                new HQLParam(m_dcurrentday, JSONFormats.TIMESTAMP),
                new HQLParam(new Date(m_dcurrentday.getTime() + 3600000L), JSONFormats.TIMESTAMP)
            };
        }
    }
    ...
    public void writeValueEdit(EReservation value) {

        if (value == null)  {
            reservation = new EReservation();
            reservation.setDatenew(m_dcurrentday);
            reservation.setChairs(2);
        } else {
            reservation = value;
        }
        
        m_timereservation.setCheckDates(m_dcurrentday, new Date(m_dcurrentday.getTime() + 3600000L));
        m_timereservation.setDate(reservation.getDatenew());
        txtCustomer.setText(reservation.getTitle());
        m_jtxtChairs.setValueInteger(reservation.getChairs());
        m_jtxtDescription.setText(reservation.getDescription());

        m_jbtnReceive.setEnabled(!reservation.isDone()); // se habilita si no se ha recibido al cliente
        txtCustomer.activate();
    }    

    public EReservation readValueEdit() throws BasicException {

        reservation.setDatenew(m_timereservation.getDate());
        reservation.setTitle(txtCustomer.getText());
        reservation.setChairs(m_jtxtChairs.getValueInteger());
        reservation.setDescription( m_jtxtDescription.getText());
        return reservation;
    }    
    ...
    private static class CompareReservations implements Comparator<EReservation> {
        public int compare(EReservation o1, EReservation o2) {
            Date d1 = o1.getDatenew();
            Date d2 = o2.getDatenew();
    ...
    private void m_jbtnReceiveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jbtnReceiveActionPerformed
        ...
        try {
            m_bd.saveData();
            m_restaurantmap.viewTables(reservation.getBusinessPartner(), reservation.getTitle());
        } catch (BasicException eD) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        EBusinessPartner c = new EBusinessPartner();
        ...
        if (c == null) {         
            ...
            txtCustomer.setText(c.getIdentifier());
            reservation.setBusinessPartner(c.getId());
        }
