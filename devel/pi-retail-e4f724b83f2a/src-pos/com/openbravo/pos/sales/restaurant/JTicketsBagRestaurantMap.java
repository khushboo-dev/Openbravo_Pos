...
import com.openbravo.pos.entities.EFloor;
import com.openbravo.pos.entities.EPlace;
...
//    private static final Icon ICO_FRE = new NullIcon(22, 22);
    ...
    private java.util.List<EPlace> places;
    private java.util.List<EFloor> floors;
    
    private JTicketsBagRestaurant m_restaurantmap;  
    private JTicketsBagRestaurantRes m_jreservations;   
    
    private EPlace m_PlaceCurrent;
    
    // State vars
    private EPlace m_PlaceClipboard;
    ...
    public JTicketsBagRestaurantMap(AppView app, TicketsEditor panelticket) {
        ...
        try {
            floors = new ServiceHQL<EFloor>(App.getInstance().getUser().getHttpSession(), EFloor.getClassEBean(), "order by name").list();
        } catch (BasicException eD) {
            floors = new ArrayList<EFloor>();
        }
        try {
            places = new ServiceHQL<EPlace>(App.getInstance().getUser().getHttpSession(), EPlace.getClassEBean(), "order by obposFloors").list();
        } catch (BasicException eD) {
            places = new ArrayList<EPlace>();
        } 
    ...
    public boolean deactivate() {
        ...
        if (viewTables()) {
            ...
            if (m_PlaceCurrent != null) {
                ...
                try {
                    dlReceipts.saveSharedTicket(m_PlaceCurrent.getTicketId(), m_PlaceCurrent.getId(), m_panelticket.getActiveTicket());
    ...
    public boolean viewTables(String customerid, String customerIdentifier) {
    ...
    public void deleteTicket() {
        ...
        if (m_PlaceCurrent != null) {
            ...
            try {
                dlReceipts.deleteSharedTicket(m_PlaceCurrent.getTicketId());
            } catch (BasicException e) {
                new MessageInf(e).show(this);
            }       
            m_PlaceCurrent.setTicketId(null);
    ...
    public void loadTickets() {
        ...
        java.util.List<ESharedTicketView> l;
        try {
            l = dlReceipts.getSharedTicketList();
        } catch (BasicException e) {
            new MessageInf(e).show(this);
            l = new ArrayList<ESharedTicketView>(); // emptylist
        }            
            
        for (EPlace table : places) {
            ESharedTicketView sharedticket = findSharedTicket(l, table.getId());
            if (sharedticket == null) {
                table.setTicketId(null);
                table.setPeople(false);                
            } else {
                table.setTicketId(sharedticket.getId());
                table.setPeople(true);
            }
        }
    }

    private ESharedTicketView findSharedTicket(java.util.List<ESharedTicketView> l, String placeid) {
        for (ESharedTicketView ticket : l) {
            if (placeid.equals(ticket.getPlace())) {
                return ticket;
            }
        }
        return null;
    ...
    private void printState() {
        ...
        if (m_PlaceClipboard == null) {
            if (customer == null) {
                ...
                // Enable all tables
                for (EPlace place : places) {
                    place.getButton().setEnabled(true);
                }
                m_jbtnReservations.setEnabled(true);
            } else {
                // receive a customer
                m_jText.setText(AppLocal.getIntString("label.restaurantcustomer", new Object[] {customerIdentifier}));
                // Enable all tables
                for (EPlace place : places) {
    ...
    private EOrder getTicketInfo(EPlace place) {

        try {
            return dlReceipts.getSharedTicket(place.getTicketId());
        } catch (BasicException e) {
    ...
    private void setActivePlace(EPlace place, EOrder ticket) {
    ...
    private class MyActionListener implements ActionListener {
        ...
        private EPlace m_place;
        
        public MyActionListener(EPlace place) {
            m_place = place;
        }
        ...
        public void actionPerformed(ActionEvent evt) {    
            ...
            if (m_PlaceClipboard == null) {  
                ...
                if (customer == null) {
                    ...
                    EOrder ticket = getTicketInfo(m_place);
                    ...
                    if (ticket == null && !m_place.hasPeople()) {
                        ...
                        ticket = new EOrder();
                        try {
                            ESharedTicket sharedticket = dlReceipts.saveSharedTicket(m_place.getTicketId(), m_place.getId(), ticket);
                            m_place.setTicketId(sharedticket.getId());
                    ...
                    if (ticket == null) {
                        ...
                        // table occupied
                        ticket = new EOrder();
                        ticket.setCustomer(customer, customerIdentifier);
                ...
                // check if the sharedticket is the same
                EOrder ticketclip = getTicketInfo(m_PlaceClipboard);
                ...
                if (ticketclip == null) {
                    ...
                    if (m_PlaceClipboard == m_place) {
                        ...
                        EPlace placeclip = m_PlaceClipboard;
                        m_PlaceClipboard = null;
                        customer = null;
                        customerIdentifier = null;
                        printState();
                        ...
                        if (ticket == null) {
                            try {
                                ESharedTicket sharedticket = dlReceipts.saveSharedTicket(m_place.getTicketId(), m_place.getId(), ticketclip);
                                m_place.setTicketId(sharedticket.getId());
                                m_place.setPeople(true);

                                dlReceipts.deleteSharedTicket(m_PlaceClipboard.getTicketId());
                                m_PlaceClipboard.setTicketId(null);
                                m_PlaceClipboard.setPeople(false);
                            } catch (BasicException e) {
                                new MessageInf(e).show(JTicketsBagRestaurantMap.this); // Glup. But It was empty.
                            }
                        ...
                        if (ticket == null) {
                            ...
                            if (JOptionPane.showConfirmDialog(JTicketsBagRestaurantMap.this, AppLocal.getIntString("message.mergetablequestion"), AppLocal.getIntString("message.mergetable"), JOptionPane.YES_NO_OPTION)
                                ...
                                try {
                                    ...
                                    ticket.setCustomer(ticketclip.getCustomer(), ticketclip.getCustomerIdentifier());
                                    }
                                    for(EOrderLine line : ticketclip.getLines()) {
                                        ticket.addLine(line);
                                    }
