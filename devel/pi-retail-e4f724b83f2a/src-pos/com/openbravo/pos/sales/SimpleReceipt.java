...
public class SimpleReceipt extends javax.swing.JPanel {
    ...
    public void setTicket(EOrder ticket, Object ticketext) {
    ...
    private void refreshTicketTaxes() {
        ...
        for (EOrderLine line : ticket.getLines()) {
            line.setTax(App.getInstance().getTaxesLogic().getTaxInfo(line.getProductTaxCategoryID(),  ticket.getOrderDate(), ticket.getCustomerEntity()));
        }
    }
    ...
    public EOrderLine[] getSelectedLines() {
        ...
        if (i >= 0) {
            ...
            List<EOrderLine> l = new ArrayList<EOrderLine>();
            
            EOrderLine line = ticket.getLine(i);
            l.add(line);
            ticket.removeLine(i);
            ...
            printTotals();
            return l.toArray(new EOrderLine[l.size()]);
        } else {
            return null;
        }
    }
    
    public EOrderLine[] getSelectedLinesUnit() {
    ...
    public void addSelectedLines(EOrderLine[] lines) {
        
        int i = findFirstNonAuxiliarLine();         
              
        EOrderLine firstline = lines[0];
        
        if (i >= 0 
                && ticket.getLine(i).getProduct() != null && firstline.getProduct() != null && ticket.getLine(i).getProduct().equals(firstline.getProduct())
                && ticket.getLine(i).getTax().getId().equals(firstline.getTax().getId())
    ...
    private void btnCustomerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCustomerActionPerformed
        ...
        if (finder.getSelectedCustomer() == null) {
            ticket.setCustomer(null, null);
        } else {
            ticket.setCustomer(finder.getSelectedCustomer().getId(), finder.getSelectedCustomer().getIdentifier());
        }
