...
public class JProductAttEdit extends javax.swing.JDialog {
    ...
    private void init(Session s) {
        ...
        attsetSave = new PreparedSentence(s,
                "INSERT INTO OBPOS_ATTRIBUTESETINSTANCE (OBPOS_ATTRIBUTESETINSTANCE_ID, OBPOS_ATTRIBUTESET_ID, DESCRIPTION) VALUES (?, ?, ?)",
                new SerializerWriteBasic(Datas.STRING, Datas.STRING, Datas.STRING));
        attinstSave = new PreparedSentence(s,
                "INSERT INTO OBPOS_ATTRIBUTEINSTANCE(OBPOS_ATTRIBUTEINSTANCE_ID, OBPOS_ATTRIBUTESETINSTANCE_ID, OBPOS_ATTRIBUTE_ID, VALUE) VALUES (?, ?, ?, ?)",
                new SerializerWriteBasic(Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING));

        attsetSent = new PreparedSentence(s,
                "SELECT OBPOS_ATTRIBUTESET_ID, NAME FROM OBPOS_ATTRIBUTESET WHERE OBPOS_ATTRIBUTESET_ID = ?",
                SerializerWriteString.INSTANCE,
                new SerializerRead() { public Object readValues(DataRead dr) throws BasicException {
                    return new AttributeSetInfo(dr.getString(1), dr.getString(2));
                }});
        attsetinstExistsSent = new PreparedSentence(s,
                "SELECT OBPOS_ATTRIBUTESETINSTANCE_ID FROM OBPOS_ATTRIBUTESETINSTANCE WHERE OBPOS_ATTRIBUTESET_ID = ? AND DESCRIPTION = ?",
                new SerializerWriteBasic(Datas.STRING, Datas.STRING),
                SerializerReadString.INSTANCE);

        attinstSent = new PreparedSentence(s, "SELECT A.OBPOS_ATTRIBUTE_ID, A.NAME, " + s.DB.CHAR_NULL() + ", " + s.DB.CHAR_NULL() + " " +
                "FROM OBPOS_ATTRIBUTEUSE AU JOIN OBPOS_ATTRIBUTE A ON AU.OBPOS_ATTRIBUTE_ID = A.OBPOS_ATTRIBUTE_ID " +
                "WHERE AU.OBPOS_ATTRIBUTESET_ID = ? " +
                "ORDER BY AU.LINENO",
            SerializerWriteString.INSTANCE,
            new SerializerRead() { public Object readValues(DataRead dr) throws BasicException {
                return new AttributeInstInfo(dr.getString(1), dr.getString(2), dr.getString(3), dr.getString(4));
            }});
        attinstSent2 = new PreparedSentence(s, "SELECT A.OBPOS_ATTRIBUTE_ID, A.NAME, AI.OBPOS_ATTRIBUTEINSTANCE_ID, AI.VALUE " +
            "FROM OBPOS_ATTRIBUTEUSE AU JOIN OBPOS_ATTRIBUTE A ON AU.OBPOS_ATTRIBUTE_ID = A.OBPOS_ATTRIBUTE_ID LEFT OUTER JOIN OBPOS_ATTRIBUTEINSTANCE AI ON AI.OBPOS_ATTRIBUTE_ID = A.OBPOS_ATTRIBUTE_ID " +
            "WHERE AU.OBPOS_ATTRIBUTESET_ID = ? AND AI.OBPOS_ATTRIBUTESETINSTANCE_ID = ?" +
            "ORDER BY AU.LINENO",
            new SerializerWriteBasic(Datas.STRING, Datas.STRING),
            new SerializerRead() { public Object readValues(DataRead dr) throws BasicException {
                return new AttributeInstInfo(dr.getString(1), dr.getString(2), dr.getString(3), dr.getString(4));
            }});
        attvaluesSent = new PreparedSentence(s, "SELECT VALUE FROM OBPOS_ATTRIBUTEVALUE WHERE OBPOS_ATTRIBUTE_ID = ?",
                SerializerWriteString.INSTANCE,
                SerializerReadString.INSTANCE);
    ...
    private void m_jButtonOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jButtonOKActionPerformed
        ...
        if (description.length() == 0) {
            ...
            if (id == null) {
                // No, create a new OBPOS_ATTRIBUTESETINSTANCE and return the ID generated
                ...
                id = StringUtils.newUUID();
                try {
                    attsetSave.exec(id, attsetid, description.toString());
                    for (JProductAttEditI item : itemslist) {
                        attinstSave.exec(StringUtils.newUUID(), id, item.getAttribute(), item.getValue());
                    }
