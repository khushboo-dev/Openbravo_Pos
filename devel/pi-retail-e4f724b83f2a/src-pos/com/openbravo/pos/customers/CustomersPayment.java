...
public class CustomersPayment extends javax.swing.JPanel implements JPanelView, BeanFactoryApp {
    ...
    private void editCustomer(EBusinessPartner customer) {

        this.customer = customer;

        txtTaxId.setText(customer.getTaxID());
        txtName.setText(customer.getName());
        txtCard.setText(customer.getSearchKey());
        txtNotes.reset();
        txtNotes.setText(customer.getDescription());
        txtMaxdebt.setText(Formats.CURRENCY.formatValue(customer.getCreditLimit()));
        txtCurdebt.setText(Formats.CURRENCY.formatValue(customer.getCreditUsed()));
        txtCurdate.setText(null); // Formats.DATE.formatValue(customer.getCurdate())
        ...
        btnPay.setEnabled(customer.getCreditUsed() > 0.0);
    }
    ...
    private void readCustomer() {
        ...
        try {
            EBusinessPartner findcustomer = dlsales.findBusinessPartner(editorcard.getText().substring(1));
            if (findcustomer == null) {
                MessageInf msg = new MessageInf(MessageInf.SGN_WARNING, AppLocal.getIntString("message.cannotfindcustomer"));
                msg.show(this);
            } else {
                editCustomer(findcustomer);
            }
    ...
    private void save() {
        ...
        customer.setDescription(txtNotes.getText());

        try {
            dlcustomers.saveBusinessPartner(customer);
    ...
    private void printTicket(String resname, EOrder ticket, EBusinessPartner customer) {
    ...
    private void btnCustomerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCustomerActionPerformed
        ...
        EBusinessPartner customerfind = finder.getSelectedCustomer();
        if (customerfind != null) {
            editCustomer(customerfind);
        }  
        editorcard.reset();
        editorcard.activate();
    ...
    private void btnPayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPayActionPerformed
        ...
        if (paymentdialog.showDialog(customer.getCreditUsed(), null)) {

            // Save the ticket
            EOrder ticket = new EOrder();
            ticket.setTicketType(EOrder.RECEIPT_PAYMENT);

            List<XOrderPayment> payments = paymentdialog.getSelectedPayments();
...
//            payments.add(new XOrderPaymentVoucher(-total, "debtpaid"));

            ticket.setPayments(payments);

            ticket.setUser(app.getUser().getUser().getUserInfo());
            // ticket.setActiveCash(app.getActiveCash().getId());
            ticket.setOrderDate(new Date());
            ticket.setCustomer(customer.getId(), customer.getIdentifier());
            ...
            EBusinessPartner c;
            try {
                c = dlsales.loadBusinessPartner(customer.getId());
                if (c == null) {
                    MessageInf msg = new MessageInf(MessageInf.SGN_WARNING, AppLocal.getIntString("message.cannotfindcustomer"));
                    msg.show(this);
                } else {
